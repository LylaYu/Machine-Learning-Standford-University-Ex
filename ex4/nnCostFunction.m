...
% Part 1 Feed forward for Cost Function
x_bias = [ones(m,1) X];
a1 = x_bias;
...
temp = (-y_v.*log(h)-(1-y_v).*log(1-h))/m;
J = sum(temp(:))+lambda/2/m*(sum(sum(Theta1(:,2:end).^2))+...
    sum(sum(Theta2(:,2:end).^2)));


% Part 2 Back Prop
delta3 = a3-y_v;    % 5000*10
delta2 = (delta3*Theta2).*[ones(m,1) sigmoidGradient(z2)];     % 5000*25
delta2 = delta2(:,2:end);

Theta1_grad = delta2'*a1/m;
Theta2_grad = delta3'*a2_bias/m;
